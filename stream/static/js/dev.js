    ...
    , function (result) {
      ...
      res.forEach(function (item, line_index) {
        ...
        if (item['type'] == checkbox) {
          ...
          draw_poly(paths, type, no, distance, jam_time);
...
function update_data(points, paths, type, no, distance) {
...
function driver_match(points, type, no, jam_time,added) {
  var steps;
      ...
      var driving_2 = new AMap.Driving({
        ...
        if (status === 'complete') {
          ...
          if (parseInt(m_distance) > 1000) {
            ...
            if (added==true){
              update_data(points, points, type, no, m_distance);
            }
            draw_poly(points, type, no, m_distance, jam_time);
            return
          }
          ...
          if (added==true){
            update_data(points, paths, type, no, m_distance);
          }
          draw_poly(paths, type, no, m_distance, jam_time);
        }
      })
    }
  })
...
function add_mark(point) {
  var marker = new AMap.Marker({
    icon: "http://a.amap.com/jsapi_demos/static/demo-0center/icons/poi-marker-red.png",
    position: point,
    offset: new AMap.Pixel(-13, -30),
    draggable: false,
  });
...
function add_point(e) {
  ...
  if (old_point!=null) {
    END_MARKER=add_mark([lng,lat]);
    var type='red';
    var no = parseInt(MARKS_NO)+1;
    var jam_time='85.564132';
    driver_match([old_point,[lng,lat]],type,no,jam_time,true);
    MARKS_NO+=1;
    old_point=[];
  }
  else{
    START_MARKER=add_mark([lng,lat]);
    old_point=[lng,lat];
  }
